...
class DiredBaseCommand:
    ...
    def _find_in_view(self, item):
        ...
        fname = re.escape(basename(os.path.abspath(item)) or item.rstrip(os.sep))
        if item[~0] == os.sep:
            # For folders - match both the arrow and the folder icon
            pattern = r'^\s*[▸▾]\s+\S+\s+'  # This will match any folder pattern
            sep = re.escape(os.sep)
        else:
            # For files - match any file pattern including custom icons
            pattern = r'^\s*≡\s+\S+\s+'  # This will match any file icon pattern
            sep = ''
        return self.view.find_all('%s%s%s' % (pattern, fname, sep))
