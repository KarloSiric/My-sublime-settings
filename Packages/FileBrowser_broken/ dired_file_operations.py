...
class DiredRenameCommitCommand(TextCommand, DiredBaseCommand):
    ...
    def run(self, edit):
        ...
        # Store the current path before renaming
        ...
        # Complete refresh of the view to prevent duplicate entries
        # This ensures a clean state with no duplicates
        self.view.run_command('dired_refresh', {
            'path': current_path,
            'to_expand': self.re_expand_new_names(),
            'reset_sels': True  # Force a complete reset of the view
        })
    ...
    def _new_name(self, line, path=None, full=False):
        ...
        new_name = self.view.substr(Region(name_point, line.b))
        
        # Remove the specific Nerd Font character "𝌆" and other non-printable characters
        new_name = new_name.replace("𝌆", "").strip()
        
        # Remove any other non-ASCII characters that might be present
        import re
        new_name = re.sub(r'[^\x20-\x7E]', '', new_name)
